...
linear_interpolation = LinearInterpolation(xs, A)
linear_ys = [linear_interpolation(x) for x in x_linspace]
plot!(x_linspace, linear_ys, label = "Linear interpolation")

cubic_interpolation = CubicSplineInterpolation(xs, A)
cubic_ys = [cubic_interpolation(x) for x in x_linspace]
plot!(x_linspace, cubic_ys, label = "Cubic spline interpolation")

quadratic_interpolation = interpolate(A, BSpline(Quadratic(Line(OnCell()))))
quadratic_ys = [quadratic_interpolation(x) for x in x_linspace]
plot!(x_linspace, quadratic_ys, label = "Quadratic spline interpolation")
